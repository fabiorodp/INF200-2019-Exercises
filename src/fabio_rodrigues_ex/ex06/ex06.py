...
"""logistic regression. The way such a model works is that you have some
function 𝑓 that you wish to learn from pairs of measurements, x_i and
target values y_i."""

"""NOTE: for the source code, we use the name coef and coef_ for the 
regression coefficients, whereas we use the name w for mathematical 
expression. This is to follow the standard set by scikit-learn."""


"""TIPS: 
- You will need matrix multiplication (the @ operator)
# A @ B # multiplication between matrizes A and B
 
- You should use broadcasting (x[:, np.newaxis])
...
- Look at the attributes field of the LogisticRegression docstring to 
see which attributes you should assign in the __init__ method."""
...
def predict_proba(coef, X):
    ...
    Estimate which class each data point in X corresponds to. This is
...
def logistic_gradient(coef, X, y):
    ...
    .. math::
        \nabla_w L(\mathbf{w}; X, \mathbf{y}) = \sum_i
        \mathbf{x}_i (y_i - \hat{y}_i),
    or, elementwise,
    .. math::
...
class LogisticRegression(BaseEstimator, ClassifierMixin):
    """A logistic regression classifier that follows the scikit-learn
    API. Note that the ``__init__`` method of scikit-learn estimators
    should not do any logic or input validation. This is all taken
    care of in the ``fit`` method.
    Parameters
    ----------
    ...
    def __init__(self, max_iter=1000, tol=1e-5,
        ...
        The ``__init__`` method of scikit-learn estimators should not do
        any logic or input validation. This is all taken care of in
        the ``fit`` method.
        Parameters
        ----------
        max_iter : int (default=1000)
            Maximum number of gradient descent iterations to run.
        tol : float (default=1e-5)
            The gradient descent iterations will converge when the
            gradient norm is less than this.
        learning_rate : float (default=0.01)
            The step-size for the gradient descent updates.
        random_state : np.random.random_state or int or None
                        (default=None)
    ...
    def _has_converged(self, coef, X, y):
        ...
        :math:`|| \mathbf{v} ||` is the norm of the vector
        :math:`\mathbf{v}`, :math:`\mathbf{w}^{(k)}` is the weights at
        iteration ``k``, and :math:`T` is the convergence tolerance (
        ``self.tol``).
        Parameters
        ----------
        coef : np.ndarray(shape=(r,))
    ...
    def _fit_gradient_descent(self, coef, X, y):
        ...
        where :math:`\mathbf{w}^{(k)}` is the coefficient vector at
        iteration ``k``, :math:`\mathbf{w}^{(k-1)}` is the coefficient
        vector at iteration k-1, :math:`\eta` is the learning rate and
        :math:`\nabla L(\mathbf{w}^{(k-1)}; X, \mathbf{y})` is the
        gradient of the loss function at iteration k-1.
        The iterative algorithm should be performed for at most
        ``self.max_iter`` iterations, or until the convergence criteria
        is reached.
        Parameters
        ----------
        coef : np.ndarray(shape=(r,))
    ...
    def fit(self, X, y):
        ...
        if any((y < 0) | (y > 1)):
            raise ValueError("Only y-values between 0 and 1 are "
                             "accepted.")
    ...
    def predict_proba(self, X):
        ...
        This function returns the probability that each datapoint
        ...
        -------
        p : np.ndarray
            A vector of probabilities. The i-th entry is the probability
            for the i-th data point belonging to the positive class.
        """
    ...
    def predict_log_proba(self, X):
        ...
        lp : np.ndarray
            ...
            probability for the i-th data point belonging to the
            positive class.
        """
        return np.log(self.predict_proba(X))

    def predict(self, X):
        """Predict whether each data point in X belongs to the positive
        class.
        Parameters
        ----------
        X : np.ndarray
        ...
        -------
        yhat : np.ndarray
            Predicted classes for the input data matrix.
            len(yhat) == len(X)
        """
...
if __name__ == "__main__":
    """Missing here 2 lines according to the tesk"""
